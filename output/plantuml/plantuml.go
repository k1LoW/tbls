...
func (p *PlantUML) OutputSchema(wr io.Writer, s *schema.Schema) error {
	...
	tmpl := template.Must(template.New(s.Name).Funcs(output.Funcs(p.config)).Parse(ts))
...
func (p *PlantUML) OutputTable(wr io.Writer, t *schema.Table) error {
	...
	tmpl := template.Must(template.New(t.Name).Funcs(output.Funcs(p.config)).Parse(ts))
	err = tmpl.Execute(wr, map[string]interface{}{
		"Table":       tables[0],
		"Tables":      tables[1:],
		"Relations":   relations,
